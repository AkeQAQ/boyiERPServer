...
@Slf4j
@RestController
@RequestMapping("/finance/supplierRoundDown")
public class FinanceSupplierRoundDownController extends BaseController {
     ...
     */
    @GetMapping("/lockById")
    @PreAuthorize("hasAuthority('finance:roundDown:list')")
    ...
    @Transactional
    @PostMapping("/del")
    @PreAuthorize("hasAuthority('finance:roundDown:del')")
    public ResponseResult delete(@RequestBody Long[] ids) throws Exception{
        try {

            List<FinanceSupplierRoundDown> olds = financeSupplierRoundDownService.listByIds(Arrays.asList(ids));
            for(FinanceSupplierRoundDown old : olds){
            ...
            boolean flag = financeSupplierRoundDownService.removeByIds(Arrays.asList(ids));

            log.info("删除抹零表信息,ids:{},是否成功：{}",ids,flag?"成功":"失败");
            if(!flag){
                return ResponseResult.fail("抹零删除失败");
            }
    ...
    public ResponseResult queryById(Long id) {
        ...
        FinanceSupplierRoundDown fsp = financeSupplierRoundDownService.getById(id);
     ...
     */
    @PostMapping("/update")
    @PreAuthorize("hasAuthority('finance:roundDown:update')")
    @Transactional
    public ResponseResult update(Principal principal, @Validated @RequestBody FinanceSupplierRoundDown fsp)
            throws Exception{

        fsp.setUpdated(LocalDateTime.now());
        fsp.setUpdateUser(principal.getName());
        fsp.setStatus( DBConstant.TABLE_FINANCE_SUPPLIER_ROUND_DOWN.STATUS_FIELDVALUE_2);
        try {
            financeSupplierRoundDownService.updateById(fsp);
     ...
     */
    @PostMapping("/save")
    @PreAuthorize("hasAuthority('finance:roundDown:save')")
    @Transactional
    public ResponseResult save(Principal principal, @Validated @RequestBody FinanceSupplierRoundDown fsp)throws Exception {
    ...
    public ResponseResult list( String searchField, String searchStatus,
        ...
        Page<FinanceSupplierRoundDown> pageData = null;
        ...
        pageData = financeSupplierRoundDownService.innerQueryByManySearch(getPage(),searchField,queryField,searchStr,searchStatusList,queryMap,searchStartDate,searchEndDate);
    ...
    public ResponseResult statusSubmit(Principal principal,Long id)throws Exception {
        FinanceSupplierRoundDown old = financeSupplierRoundDownService.getById(id);
        if(old.getStatus()!=DBConstant.TABLE_FINANCE_SUPPLIER_ROUND_DOWN.STATUS_FIELDVALUE_1){
            return ResponseResult.fail("状态已被修改.请刷新");
        }
        FinanceSupplierRoundDown fsp = new FinanceSupplierRoundDown();
        fsp.setUpdated(LocalDateTime.now());
        fsp.setUpdateUser(principal.getName());
        fsp.setId(id);
        fsp.setStatus(DBConstant.TABLE_FINANCE_SUPPLIER_ROUND_DOWN.STATUS_FIELDVALUE_2);
        financeSupplierRoundDownService.updateById(fsp);
        log.info("财务模块-抹零模块-审核通过内容:{}",fsp);
    ...
    public ResponseResult statusSubReturn(Principal principal,Long id)throws Exception {
        ...
        FinanceSupplierRoundDown old = financeSupplierRoundDownService.getById(id);
        if(old.getStatus()!=DBConstant.TABLE_FINANCE_SUPPLIER_ROUND_DOWN.STATUS_FIELDVALUE_2 &&
                old.getStatus()!=DBConstant.TABLE_FINANCE_SUPPLIER_ROUND_DOWN.STATUS_FIELDVALUE_3
        ){
            return ResponseResult.fail("状态已被修改.请刷新");
        }
        ...
        fsp.setUpdateUser(principal.getName());
        fsp.setId(id);
        fsp.setStatus(DBConstant.TABLE_FINANCE_SUPPLIER_ROUND_DOWN.STATUS_FIELDVALUE_1);
        financeSupplierRoundDownService.updateById(fsp);
        log.info("财务模块-抹零模块-审核通过内容:{}",fsp);
    ...
    @PreAuthorize("hasAuthority('finance:roundDown:valid')")
    public ResponseResult statusPassBatch(Principal principal,@RequestBody Long[] ids) {
        ArrayList<FinanceSupplierRoundDown> lists = new ArrayList<>();
        ...
        for (Long id : ids){
            ...
            fsp.setUpdateUser(principal.getName());
            fsp.setId(id);
            fsp.setStatus(DBConstant.TABLE_FINANCE_SUPPLIER_ROUND_DOWN.STATUS_FIELDVALUE_0);
            lists.add(fsp);

        }
        financeSupplierRoundDownService.updateBatchById(lists);
    ...
    public ResponseResult statusReturn(Principal principal,Long id)throws Exception {
        ...
        FinanceSupplierRoundDown old = financeSupplierRoundDownService.getById(id);
        if(old.getStatus()!=DBConstant.TABLE_FINANCE_SUPPLIER_ROUND_DOWN.STATUS_FIELDVALUE_0){
            return ResponseResult.fail("状态已被修改.请刷新");
        }
        FinanceSupplierRoundDown fsp = new FinanceSupplierRoundDown();
        fsp.setUpdated(LocalDateTime.now());
        fsp.setUpdateUser(principal.getName());
        fsp.setId(id);
        fsp.setStatus(DBConstant.TABLE_FINANCE_SUPPLIER_ROUND_DOWN.STATUS_FIELDVALUE_3);
        financeSupplierRoundDownService.updateById(fsp);
